  ...
  <div class="wrap">
    ...
    <ol class="steps stages">
      <li><h4>Text extraction, with native, OCR and vision routing</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>worker/extract.py</code> · <b>Covered by</b> the scanned half of the synthetic corpus in <code>make eval</code>; every residual miss in the changelog traces back to a Tesseract misread on one of those pages</p></li>
      <li><h4>Presidio de-identification, with a reversible per-tenant PHI map</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>worker/deid.py</code>, map storage in <code>worker/store.py</code> · <b>Covered by</b> <code>tests/test_deid.py</code> (real Presidio); <code>deid_recall</code> and <code>deid_recall_strict</code> in <code>eval/run_eval.py</code></p></li>
      <li><h4>Clinical fact extraction</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>worker/annotate.py</code> · <b>Covered by</b> <code>tests/test_annotate.py</code>; <code>eval/run_extraction_eval.py</code> (<code>make eval-extraction</code>), precision, recall and F1 per fact kind with assertion required for problems</p></li>
      <li><h4>Index</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>worker/store.py</code> · <b>Covered by</b> the <code>deidentified</code> assertion in the <code>chunk_embed</code> node; the vector-literal and 384-dimension guards in <code>tests/test_retrieval.py</code>; <code>deid_recall_strict</code>, which scans every stored chunk for every injected identifier</p></li>
      <li><h4>Retrieval scoped to (tenant, patient)</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>worker/retrieval.py</code>, namespace in <code>worker/reid.py</code> · <b>Covered by</b> <code>tests/test_retrieval.py::test_query_is_hard_filtered_by_tenant_and_patient</code> and thirteen more; <code>cross_patient_leaks</code> in the eval, 0 in every run</p></li>
      <li><h4>Answer</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>worker/llm.py</code> · <b>Covered by</b> <code>answer_accuracy</code> (strict: the first gold item verbatim) and <code>answer_accuracy_lenient</code> (drug and dose, or the lab value, for every gold item)</p></li>
      <li><h4>Validation</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> the <code>validate</code> node of <code>worker/graph.py</code>, judge in <code>worker/llm.py</code> · <b>Covered by</b> <code>queries_failed_validation</code> in the eval; the judge prompt measured on qwen2.5:7b at 1.0 for a supported answer and 0.0 for an unsupported one</p></li>
      <li><h4>Single escalation</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>after_validate</code> in <code>worker/graph.py</code> · <b>Covered by</b> <code>queries_escalated</code> and <code>queries_failed_validation</code></p></li>
      <li><h4>Re-identification, for the authorized caller only</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>worker/reid.py</code> · <b>Covered by</b> <code>tests/test_reid.py</code>, including the case where a question about one clinician could name another</p></li>
      <li><h4>Audit write</h4>
        ...
        <p class="stage-meta"><b>Lives in</b> <code>db/init.sql</code>, writes in <code>worker/store.py</code> and <code>gateway/main.py</code> · <b>Covered by</b> <code>REVOKE UPDATE, DELETE ON audit_log FROM app_rw</code>; UPDATE and DELETE as <code>app_rw</code> return permission denied</p></li>
    </ol>
  </div>
</section>
  ...
  <div class="wrap">
    ...
    <ol class="steps stages">
      <li><h4>Nothing reaches a model until the state is de-identified</h4>
        ...
        <p class="stage-meta"><b>Enforced</b> by <code>assert state["deidentified"]</code> in <code>worker/graph.py</code> (annotate, chunk_embed, generate); Presidio and the added recognizers in <code>worker/deid.py</code>; <code>CHECKPOINTER=none</code><br><b>Tested</b> by <code>tests/test_deid.py</code> (real Presidio, thirty-plus cases: labelled MRNs, phones with extensions, address lines, titles, frequencies kept, calendar dates removed, round trip through <code>restore</code>); <code>deid_recall</code> and <code>deid_recall_strict</code> in <code>eval/run_eval.py</code></p></li>
      <li><h4>The PHI map never enters the vector store</h4>
        ...
        <p class="stage-meta"><b>Enforced</b> in <code>worker/store.py</code> (chunks are written from the de-identified text; the map only ever through <code>pgp_sym_encrypt</code>) and <code>worker/reid.py</code><br><b>Tested</b> by <code>deid_recall_strict</code>, which scans every stored chunk for every component of every injected identifier (1.000 on the 20-record set); <code>tests/test_reid.py::test_restore_uses_the_right_document_and_only_needed_tokens</code></p></li>
      <li><h4>Retrieval is hard-filtered on patient</h4>
        ...
        <p class="stage-meta"><b>Enforced</b> in <code>worker/retrieval.py::hybrid</code>; the <code>tenant_isolation</code> policy on every PHI table in <code>db/init.sql</code><br><b>Tested</b> by <code>tests/test_retrieval.py::test_query_is_hard_filtered_by_tenant_and_patient</code>, which checks the SQL and the bind parameters; <code>cross_patient_leaks</code> in <code>eval/run_eval.py</code>, which checks every answer for every other patient's injected identifiers, 0 in every run</p></li>
      <li><h4>Tenant identity comes from the credential, never a client-supplied field</h4>
        ...
        <p class="stage-meta"><b>Enforced</b> in <code>gateway/auth.py</code>; per-transaction <code>app.tenant_id</code> in <code>gateway/main.py</code> and <code>worker/store.py</code>; <code>app_rw</code> with its password set by <code>db/02_app_role.sh</code><br><b>Tested</b> by <code>tests/test_auth.py</code> (hashing, pepper rotation, revoked and tampered keys, rate limits); the cross-tenant probes below; <code>eval/run_eval.py</code> refuses to run as a superuser or a BYPASSRLS role, so its leak count cannot be flattered by the same mistake twice</p></li>
      <li><h4>The audit log is append-only</h4>
        ...
        <p class="stage-meta"><b>Enforced</b> by <code>REVOKE UPDATE, DELETE ON audit_log FROM app_rw</code> in <code>db/init.sql</code> (the blanket grant above it had included UPDATE; that was the bug)<br><b>Tested</b> by running UPDATE and DELETE on <code>audit_log</code> as <code>app_rw</code>: permission denied</p></li>
    </ol>
  </div>
</section>
...
<section>
  <div class="wrap prose">
    ...
    <p>The fix is small. The application connects as <code>app_rw</code>, a plain login role the policies apply to. Its password comes from the environment through <code>db/02_app_role.sh</code>, because Postgres runs init <code>.sql</code> files without environment substitution and <code>.sh</code> files with it, which also removed the hard-coded password from <code>init.sql</code>. The role gets DELETE on <code>chunks</code> and <code>phi_tokens</code>, which re-ingest needs, and loses UPDATE and DELETE on <code>audit_log</code>.</p>
    <p>Then the probes, run against the live stack and kept: key B gets 404 on tenant A's job; an <code>app_rw</code> session with <code>app.tenant_id</code> set to A sees zero of B's rows; a cross-tenant INSERT while set to A violates the policy. And <code>eval/run_eval.py</code> now refuses to run as a superuser or a BYPASSRLS role.</p>
  ...
  <div class="wrap grid-2">
    ...
    <div>
      ...
      <ul>
        ...
        <li>Real data, ever. <code>TODO.md</code> in the repository lists what would have to be true first.</li>
      </ul>
    </div>
  </div>
</section>
