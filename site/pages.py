...
from build import ONE_LINER, PROFILE, REPO, SHORT, gh, src
...
<pre><code>git clone {REPO} &amp;&amp; cd arbiterai</code></pre>
    <p>Every command below runs from the root of the clone; the targets are in the {gh("Makefile")}.</p>
    <h3>Prerequisites</h3>
    <ul>
      <li>Docker (validated with Colima on Apple Silicon).</li>
      <li>Python 3.12. {gh("Makefile", "make venv")} creates <code>.venv/</code>; 3.14 is too new for spaCy and Presidio.</li>
...
{gh("Makefile", "make up")}              # postgres (pgvector), redis, embeddings, gateway, worker
{gh("Makefile", "make synth")} N=20      # 20 synthetic records with injected fake identifiers -> platform/data/synthetic/
{gh("Makefile", "make bootstrap")}       # dev tenant + one API key -> platform/.env.dev-tenant (git-ignored)</code></pre>
    <p>{gh("Makefile", "make bootstrap")} prints the tenant id and the key once. The gateway listens on <code>127.0.0.1:8080</code>.</p>
...
{gh("Makefile", "make test")}                       # pytest, no Docker needed: 130 passed, 2 skipped, 2 xfailed</code></pre>
    ...
    <p>Gold questions live in the manifest. {src("scripts/make_synthetic_docs.py")} writes a <code>gold_qa</code> list per record (<code>{{"q": …, "a": …}}</code>) next to <code>injected_phi</code> and <code>gold_facts</code>. Add a question there, regenerate with {gh("Makefile", "make synth N=20")}, and {src("eval/run_eval.py", "run_eval.py")} picks it up. The first gold item is what the strict metric checks verbatim, so put the most specific value first.</p>

    <h2 id="data">Data</h2>
    <p>Every record that ships with the reference implementation is synthetic. No real patient has ever been near it. {src("scripts/make_synthetic_docs.py")} generates one-page discharge summaries with Faker, seeded so the corpus is reproducible, and plants six identifiers per record that the de-identification eval must remove: a name, a date of birth, a medical record number, a phone number with an extension, a street address, and an attending physician. Each record also carries the gold facts: two diagnoses, three medications, an HbA1c, a blood pressure, an LDL, two negated findings, one family-history condition, and an allergy with reaction or a note of no known allergies.</p>
...
<section class="on-white">
  <div class="wrap grid-2">
    <div>
      ...
      <p>The code, the tests and the eval harness are open source. Apache-2.0 licensed.</p>
      <p class="author-links"><a href="mailto:hello@arbiterai.tech">hello@arbiterai.tech</a><a href="{REPO}">the repository</a><a href="{PROFILE}" rel="me">GitHub profile</a></p>
      <h2>Why</h2>
