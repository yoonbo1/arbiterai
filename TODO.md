...
11. ~~`contains_phi` ignores dates and locations in the output check. Decide (Safe Harbor says they are identifiers) and document the decision on the security page either way.~~ Done 2026-09-05: calendar-like dates and address-recognizer locations now count; bare state names, frequencies and durations still pass; rationale in `platform/docs/HIPAA_CONTROLS.md` "Output leak check". Still to do: one sentence on the security page (site/).
12. ~~Move the custom Presidio recognizers into a delimited module~~ (done 2026-09-05: `platform/worker/recognizers/`, one module each) and open the four upstream PRs the brief lists (labelled MRN, NANP with extension, title-anchored names, dosing-frequency date filter). **The PRs are written up in `platform/docs/UPSTREAM.md` but not opened.** Each PR is a portfolio item and links from its post. Also: `site/pages_blog.py` still names `_address_recognizer()` in `worker/deid.py`; point it at `worker/recognizers/address.py`.
...
14. ~~The open 0.992: OCR-corrupted identifier labels (`MAN:` for `MRN:`). Edit-distance tolerance on labels, or treat any 6 to 10 digit run on the demographics line as an identifier.~~ Done 2026-09-05, both: edit-distance-1 and spacing label variants at 0.7, and a 6 to 10 digit run on a line with a demographics cue at 0.5 (`worker/recognizers/mrn.py`). The OCR eval has not been re-run yet; expectation is 1.000.
...
24. A schema test that runs every SQL statement the worker and gateway issue against a throwaway Postgres built from `init.sql` plus migrations (the merge of 2026-09-05 shipped an `UPDATE jobs ... result=` against a column that no longer existed; unit tests with a fake connection cannot catch that, the eval did). Ollama as a managed service; cache OCR so `choose_route` does not run Tesseract twice; asyncpg pool validation after a Postgres restart; retry with backoff for transient model errors; split `env_file` secrets per service; widen `api_keys.key_prefix`; `make venv` never installs presidio, spaCy or psycopg (they were pre-installed on this machine; CI installs the gateway, worker and dev requirements instead, so mirror that in the Makefile); verify the `gpu` profile, `vlm` route and Postgres checkpointer on a GPU box.
...
- [x] **OCR-corrupted identifier labels leave identifiers bare.** Tesseract read `MRN:` as `MAN:` on one scanned page and the 7-digit MRN survived de-identification (recall 0.992 on the regenerated corpus). Add edit-distance tolerance for identifier labels, or treat any 6 to 10 digit run on the demographics line as an identifier. (Both added 2026-09-05 in `worker/recognizers/mrn.py`; eval not re-run yet.)
...
- [x] The output leak check (`contains_phi`) deliberately ignores `DATE_TIME` and `LOCATION`, so a raw date or address in an answer would pass. Dates and geography smaller than a state are Safe Harbor identifiers. Decide whether that is acceptable for the product and document it, or tighten the check. (Tightened 2026-09-05: calendar-like dates and address-recognizer locations count; documented in `platform/docs/HIPAA_CONTROLS.md`.)
